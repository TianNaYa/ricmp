...
static inline char *ikcp_encode08u(char *p, unsigned char c)
{
	*(unsigned char*)p++ = c;
	return p;
}
...
static inline const char *ikcp_decode08u(const char *p, unsigned char *c)
{
	*c = *(unsigned char*)p++;
	return p;
}
...
static void ikcp_ack_push(ikcpcb *kcp, IUINT32 sn, IUINT32 ts)
{
	...
	if (newsize > kcp->ackblock) 
	{
		...
		if ((acklist = (IUINT32*)ikcp_malloc(newblock * sizeof(IUINT32) * 2)) == NULL) 
		{
			abort();
		}
...
static char *ikcp_encode_seg(char *ptr, const IKCPSEG *seg)
{
	...
	ptr = ikcp_encode08u(ptr, (IUINT8) seg->cmd);
